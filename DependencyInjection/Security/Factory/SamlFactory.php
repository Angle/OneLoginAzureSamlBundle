...
namespace Angle\OneLoginAzureSamlBundle\DependencyInjection\Security\Factory;
...
use Angle\OneLoginAzureSamlBundle\Security\Firewall\SamlListener;
use Angle\OneLoginAzureSamlBundle\Security\Http\Authentication\SamlAuthenticationSuccessHandler;
use Angle\OneLoginAzureSamlBundle\Security\Http\Authenticator\SamlAuthenticator;
...
class SamlFactory implements SecurityFactoryInterface, AuthenticatorFactoryInterface
{
    ...
    {
        trigger_deprecation('angle/oneloginazuresaml-bundle', '2.1', 'Usage of security authentication listener is deprecated, option "security.enable_authenticator_manager" should be set to true.');
    ...
    {
        $container->setDefinition('angle_one_login_azure_saml.user_created_listener.'.$firewallName, new ChildDefinition(UserCreatedListener::class))
            ->replaceArgument(1, $config['persist_user'])
            ->addTag('angle.saml_user_listener')
            ->addTag('kernel.event_listener', ['event' => UserCreatedEvent::class])
        ;

        $container->setDefinition('angle_one_login_azure_saml.user_modified_listener.'.$firewallName, new ChildDefinition(UserModifiedListener::class))
            ->replaceArgument(1, $config['persist_user'])
            ->addTag('angle.saml_user_listener')
